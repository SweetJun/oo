<template>
  <div>
    <el-form class="box_260" :model="formData" :rules="rules" ref="formData" inline label-position="top">
      <nf-form-wrap title="客户管理" :on-save-btn-click="onSaveBtnClick">
        <div class="clearfix">
      ...
      <nf-form-wrap class="photos" title="附件信息" :on-save-btn-click="onSaveBtnClick" v-if="formData.type==='enterprise'">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        id_card: [{ required: true, message: '请输入身份证号/统一社会信用代码', trigger: 'blur' }],
      ...
      $get(`${api.client}/${id}`).then(res => {
        ...
        if (res.images.business_license) {
          res.images.business_license.forEach(item => {
            this.fileList.push(`${api.client}/${this.edit_id}/type/business_license/image/${item}?${this.formData.update_time}`)
          })
        }
        this.getDet = true;
      })
      ...
      this.$refs['formData'].validate((valid) => {
        if (valid) {
          ...
          if (this.isEditing) { // 判断 编辑 和 新增
            let apiUrl = `${api.client}/${this.edit_id}`
            ...
            $post(api.client, this.formData).then(() => {
