      ...
      <el-form :model="formData" ref="ruleForm" :rules="rules" inline label-position="top">
       <el-form-item label="起运地：" prop="from">
        ...
        <div class="price-table">
          <el-table
            ref="price-table"
            :data="formData.provider_rule_specifics"
            border
            style="width: 100%"
            ...
            <el-table-column min-width="100">
              ...
              <template slot-scope="scope">
                <el-form-item :prop="'provider_rule_specifics.' + scope.$index + '.calculate_mode'">
            ...
            <el-table-column min-width="100">
              ...
              <template slot-scope="scope">
                <el-form-item
                  :prop="'provider_rule_specifics.' + scope.$index + '.price'"
                  :rules="[
            ...
            <el-table-column label="里程数" min-width="100">
              <template slot-scope="scope">
                <el-form-item
                  :prop="'provider_rule_specifics.' + scope.$index + '.mileage'"
            ...
            <el-table-column min-width="140">
              ...
              <template slot-scope="scope">
                <el-form-item
                  :prop="'provider_rule_specifics.' + scope.$index + '.dateRange'"
                  :rules="[
  ...
  data() {
    return {
      ...
      rules: {
        ...
        brandObj: [{ required: true, message: '请选择商品车品牌', trigger: 'change' }]
      }
    }
  },
  methods: {
    ...
    async remoteMethod(query, key) {
      if (query.trim() !== '') {
        this.loading = true
        let res = await $get(`${api.searchItemAutocomplete}/${key}`, { keyword: query })
        if (key === 'region_from') {
          this.options_f = res
        } else {
          this.options_t = res
    ...
    // 添加时间段
    addTimeRange() {
      this.formData.provider_rule_specifics.push({
        calculate_mode: 'price_number',
        price: '',
        mileage: '',
        dateRange: [], // 有效期
    ...
    // 删除时间段
    delTableRow(index) {
      if (this.formData.provider_rule_specifics.length === 1) {
        this.$message.warning('至少保留一项！')
        return
      }
      this.confirm('删除后，该有效期对应的运单收入将会更新为0元，请谨慎删除！', '删除确认').then(() => {
        this.formData.provider_rule_specifics.splice(index, 1)
      }).catch(() => {})
    },
    // 获取详情
    getDetail(id) {
      $get(`${api.providerRule}/${id}`).then(res => {
        ...
        this.formData.provider_rule_specifics = res.provider_rule_specifics
        this.getVehicleOptions()
      })
    },
    // 保存
    onSaveBtnClick(loading) {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          let provider_rule_specifics = _.cloneDeep(this.formData.provider_rule_specifics)
          provider_rule_specifics.forEach(item => {
          ...
          let params = {
            ...
            provider_rule_specifics: provider_rule_specifics
          }
          loading(true)
          if (this.isEditing) { // 判断 编辑 和 新增
            this.confirm('该时间段内的运单将重新计算产值收入，请谨慎修改！', '运单产值更新提示').then(() => {
              let apiUrl = `${api.providerRule}/${this.edit_id}`
            ...
            $post(api.providerRule, params).then(() => {
              loading(false)
