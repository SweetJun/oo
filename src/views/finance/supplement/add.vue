<template>
  <div v-loading="pageLoading">
    <nf-form-wrap title="补款单管理" :show-footer="false">
    ...
    <nf-form-wrap title="费用信息" :show-footer="false">
      <command ref="commands" :expense-data="expenseData"></command>
    </nf-form-wrap>
...
export default {
  ...
  computed: {
    bankcardTotal() {
      if (this.$refs['commands'].payMethodTotal) {
        return this.$refs['commands'].payMethodTotal.bank_card
      }
      return 0
    },
    oilcardTotal() {
      if (this.$refs['commands'].payMethodTotal) {
        return this.$refs['commands'].payMethodTotal.oil_card
      }
      return 0
    },
    wjyTotal() {
      if (this.$refs['commands'].payMethodTotal) {
        return this.$refs['commands'].payMethodTotal.wjy
      }
      return 0
    },
    defaultTotal() {
      if (this.$refs['commands'].payMethodTotal) {
        return this.$refs['commands'].payMethodTotal.default
      }
      return 0
    },
    borrowTotal() {
  ...
  methods: {
    ...
    // 获取计费策略匹配，中途补款没有规则，所以方法名保持不变，处理过程改一下
    getExpenseRule() {
      let result = []
      if (!this.isEditing) {
        ...
        this.command_bills.forEach(item => {
          ...
          this.$set(item, 'command_id', item.id)
          this.$set(item, 'selectRule', {
            ...
            cost: billItem.expense_items.map(expense => {
              ...
              let _cost = {
                ...
                as_borrowing: expense.as_borrowing,
                pay_methods: methods
              }
    ...
    onSaveBtnClick(loading) {
      ...
      let bill = {
        ...
        bill_type: 'dispatch_supplement',
        amount: this.moneyYuanToCent(this.borrowTotal),
        repayment: this.moneyYuanToCent(this.formData.repayment),
        final_amount: this.moneyYuanToCent(this.borrowTotal - Number(this.formData.repayment))
      }
        ...
        if (command.selectRule.cost && command.selectRule.cost.length > 0) {
          ...
          costs.forEach(cost => {
            let expense_item = {
              ...
              as_borrowing: cost.as_borrowing,
              pay_methods: []
            }
      ...
      this.$refs['baseRuleForm'].validate((valid) => {
        if (valid) {
          loading(true)
          if (this.isEditing) {
            $put(`${api.dispatchBillSupplement}/${this.edit_id}`, postParams).then(() => {
              loading(false)
            ...
            $post(api.dispatchBillSupplement, postParams).then(() => {
              loading(false)
    ...
    getDetail(id) {
      this.pageLoading = true
      $get(`${api.dispatchBillSupplement}/${id}`).then(res => {
        const bill = res.bill
        ...
        this.formData = {
          ...
          repayment: this.moneyYuanToCent(bill.amount - bill.final_amount),
