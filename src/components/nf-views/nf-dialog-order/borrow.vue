<template>
  <nf-dialog
    ...
    class="nf-dialog"
  >
    <div class="receipt-content print-content" slot="content" id="printBorrow">
      <div class="header">
        <p>借支单号：{{content.bill.bill_number}}</p>
        ...
        <tr>
          ...
          <td colspan="7" style="text-align:left;padding-left:10px;line-height:18px;">
            ...
            <span
              ...
              :key="key"
            >{{item.expense_item_display}}：{{moneyCentToYuan(item.sumMoney)|toThousandFilter}}元；</span>）
          </td>
        </tr>
        <tr v-if="Number(content.bill.amount-content.bill.final_amount)>0">
