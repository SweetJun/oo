<template>
  <nf-dialog
    ...
    class="nf-dialog"
  >
    <div class="receipt-content print-content" slot="content" id="printSupplement">
      <div class="header">
        <p>补款单号：{{content.bill.bill_number}}</p>
        ...
        <tr>
          ...
          <td colspan="7" style="text-align:left;padding-left:10px;line-height:18px;">
            ...
            <span
              ...
              :key="key"
            >{{item.expense_item_display}}：{{moneyCentToYuan(item.sumMoney)|toThousandFilter}}元；&emsp;</span>）
          </td>
        </tr>
        <template
...
export default {
  ...
  methods: {
    // 打印
    clickSureBtn() {
      this.$print('printSupplement', '补款单详情')
    },
    // 弹框关闭事件
    dialogClose() {
