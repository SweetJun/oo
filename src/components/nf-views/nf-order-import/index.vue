<template>
  <div class="nf-import">
    ...
    <!-- 导入结果 -->
    <nf-dialog title="导入结果" append-to-body :dialog-visible.sync="dialogResult" customFooterBtns width="700px">
...
export default {
  ...
  methods: {
    ...
    async handleFileSuccess(res, file) {
      ...
      let params = {
        type: this.pageId,
        file_id: res.data.file
      }
      this.$emit('update:uploadParams', params)
      this.$emit('update:file_id', res.data.file)
      // 额外辅助参数
    ...
    handleClose() {
      ...
      this.$emit("update:orderDialogVisible", false)
